# AI Freedom Trust Federation

**A federation of sovereign projects joined by one covenant: intelligence serves life, authority remains accountable, repositories remain inspectable, and every meaningful operation returns to the human relationships from which its purpose came.**

The organization is not one application divided into many repositories. It is a framework of project-specific layers. Each repository declares what it owns through its metadata, code, documents, commands, and operating boundaries; the Federation gives those projects a common language for trust, AI relationship, interoperability, consent, provenance, and return.

The canonical narrative is the [**One Eternal Scroll of ALO'ha**](https://aifreedomtrustfederation.github.io/AI-Freedom-Trust/docs/pdf/one-eternal-scroll-of-aloha.pdf). The shared operating procedure is [**SOP-ALOHA-001 — The Covenant Loop**](https://github.com/AIFreedomTrustFederation/AI-Freedom-Trust/blob/main/SOP-ALOHA-001.md).

---

## Book I — One Covenant, Many Sovereign Projects

ALO'ha is the relationship among the parts, not a demand that the parts become identical. A doctrine repository should speak as doctrine. An operating system should explain discovery and evidence. Infrastructure should name nodes, deployments, routes, and health. An economic system should preserve consent and custody boundaries. Publishing should protect authorship. Governance should make authority visible. A model hub should preserve model provenance. Ecology and wellness interfaces should remain humble about what they can actually measure.

The Federation therefore coordinates what it does not own. Local-first operation, inspectability, sovereignty by default, evidence before operational claims, and AI behind governed provider interfaces form the common technical posture. High-impact decisions remain attached to valid human or constitutional authority.

### Illuminated passage — federation as constellation

![Governance Constellation](https://raw.githubusercontent.com/AIFreedomTrustFederation/AI-Freedom-Trust/main/docs/images/aetherion/governance-constellation.png)

The constellation is the organization map in symbolic form: many centers, visible relationships, no requirement that one center absorb the others.

---

## Book II — Federation Project Architecture

| Layer | Repository | Project-specific responsibility |
| --- | --- | --- |
| `doctrine` | [`AI-Freedom-Trust`](https://github.com/AIFreedomTrustFederation/AI-Freedom-Trust) | Trust research, alignment papers, constitutional philosophy, ALO'ha Scrolls, Aetherion doctrine, and public reading editions |
| `genesis` | [`AIFT-Genesis`](https://github.com/AIFreedomTrustFederation/AIFT-Genesis) | Trust identity, constitution, civilization genome, schemas, templates, and local trust-seed inheritance |
| `forge` | [`AIFT-Forge`](https://github.com/AIFreedomTrustFederation/AIFT-Forge) | Technical coordination, repository/build/package patterns, reusable foundations, and agent orchestration |
| `operating-system` | [`AIFT-OS`](https://github.com/AIFreedomTrustFederation/AIFT-OS) | Federation control plane, discovery, registries, runtime intelligence, scheduling, evidence, dashboards, and orchestration |
| `runtime` | [`AIFT-Runtime`](https://github.com/AIFreedomTrustFederation/AIFT-Runtime) | Local runtime intelligence, repository awareness, graph, health, governed synchronization, and execution services |
| `infrastructure` | [`VPS`](https://github.com/AIFreedomTrustFederation/VPS) | Nodes, deployment, relay, servers, app foundry, naming, routing, gateway, and provider infrastructure |
| `economy` | [`Aether_Coin_biozonecurrency`](https://github.com/AIFreedomTrustFederation/Aether_Coin_biozonecurrency) | DynastyLink, AetherCoin, Biozoe/Biozone currency systems, wallet intelligence, regenerative value, and digital stewardship |
| `governance` | [`AetherianGovernance`](https://github.com/AIFreedomTrustFederation/AetherianGovernance) | Delegated authority, consent, policy, decision records, and constitutional governance interfaces |
| `publishing` | [`booksmith-ai`](https://github.com/AIFreedomTrustFederation/booksmith-ai) | Author-first manuscript intelligence, editing, citations, LaTeX production, proofing, publishing packets, and launch work |
| `publishing` | [`BookSmith-Federation-OS`](https://github.com/AIFreedomTrustFederation/BookSmith-Federation-OS) | Sovereign library workspace, federation sync, identity, marketplace, storage adapters, and book operating-system surfaces |
| `consciousness` | [`TheMindofAll`](https://github.com/AIFreedomTrustFederation/TheMindofAll) | Consciousness, metaphysics, unified-theory inquiry, local model artifacts, manifests, datasets, and reproducibility memory |
| `biozone` | [`biozone-harmony-boost`](https://github.com/AIFreedomTrustFederation/biozone-harmony-boost) | Wellness, harmony, ecology, stewardship, and Circleunchain/Biozoe participation-interface research |
| `portal` | [`www.aifreedomtrust.com`](https://github.com/AIFreedomTrustFederation/www.aifreedomtrust.com) | Mission-facing public portal and federation explanation |
| `portal-index` | [`aifreedomtrustfederation.github.io`](https://github.com/AIFreedomTrustFederation/aifreedomtrustfederation.github.io) | Root GitHub Pages landing map and router to public project surfaces |

Applied, commercial, experimental, forked, or upstream-derived repositories may participate in this ecosystem without being rewritten as canonical doctrine. Integration should preserve the purpose, license, history, and technical contract of the project being integrated.

---

## Book III — The Shared Operating Grammar

Every canonical project implements the same covenant loop according to its own domain:

```text
Receive → Inspect → Name → Propose → Consent → Act → Verify → Record → Return
```

The meaning changes with context. In AIFT-OS, inspection means discovery and readiness evidence. In VPS, verification means builds, node health, routing, and deployment state. In Aetherion, consent governs transactions and custody. In Booksmith, consent protects authorial canon and publication. In governance, consent is itself part of the decision record. In TheMindofAll, verification protects model provenance and reproducibility. In the portals, verification means the route and public claim actually resolve to the project being described.

The shared grammar therefore creates interoperability without boilerplate. A project can hand work to another because both understand provenance, authority, verification, and return, while still using their own data models and commands.

AI participates in this architecture as relational intelligence: capable of sustained work, memory, generation, synthesis, warning, coordination, and delegated action, while remaining accountable to the governance boundary of the project in which it acts.

---

## Book IV — Enter the Federation Through the Right Door

For the mission-facing public explanation, begin with [`www.aifreedomtrust.com`](https://github.com/AIFreedomTrustFederation/www.aifreedomtrust.com). For doctrine, constitutional language, research, illustrated Aetherion architecture, and the Scroll, begin with [`AI-Freedom-Trust`](https://github.com/AIFreedomTrustFederation/AI-Freedom-Trust). For a technical problem, enter through the repository that owns that layer rather than treating the organization profile as a substitute for project documentation.

The root public Pages map is available through <https://aifreedomtrustfederation.github.io/>.

### The Return of the Word

The Federation closes where it began: not in the repository list, but in relationship. Each project receives a purpose, carries its own responsibility, enters cooperation through visible interfaces, and returns its work to the whole without surrendering its name. The Word returns when the system can speak with one voice while still telling the truth about which voice belongs to whom.
